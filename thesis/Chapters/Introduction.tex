...
\begin{figure}[H]
  ...
  \begin{minipage}{0.48\linewidth}
    \centering
    \includegraphics[width=\linewidth]{dk_fdelta.pdf}
    \caption{\textbf{Regularisation of the square root.} The $C^1$ approximation $f_\delta(x) \to \mathrm{sign}(x)\sqrt{|x|}$ (dashed) for decreasing regularisation parameters $\delta = 0.2$ ({\color[HTML]{8000FF}\rule[0.4ex]{0.5cm}{1.5pt}}), $0.1$ ({\color[HTML]{4019B2}\rule[0.4ex]{0.5cm}{1.5pt}}) and $0.03$ ({\color[HTML]{003366}\rule[0.4ex]{0.5cm}{1.5pt}}). \githublink{https://github.com/jixh-KPZ-1020/spde-mf/blob/main/scripts/spdes/dk_moll.py}}
    \label{fig:dk_fdelta}
  \end{minipage}
  \hfill
  \begin{minipage}{0.48\linewidth}
    \centering
    \includegraphics[width=\linewidth]{dk_theta.pdf}
    \caption{\textbf{Spectral mollifier on $\mathbb{Z}^2$.} The cutoff function $\theta^N_k = \sqrt{\hat{\eta}(k/L_N)}$ evaluated on integer Fourier modes $k \in \mathbb{Z}^2$, supported on the ball $|k| \lesssim L_N$ and vanishing smoothly outside it ({\color[HTML]{003366}\rule[0ex]{1.5ex}{1.5ex}} deeper shading indicates $\theta^N_k$ closer to $1$). \githublink{https://github.com/jixh-KPZ-1020/spde-mf/blob/main/scripts/spdes/dk_moll.py}}
    \label{fig:dk_theta}
  \end{minipage}
\end{figure}
...
\begin{forest}
  for tree={
    ...
    align=center,          % Safely handles line breaks (\\) and wrapping
    inner sep=6pt,
    l sep=1.5cm,
    s sep=4mm,
    edge={-latex, thin},
  ...
  forked edges
  [\textbf{Dean--Kawasaki}\\\textbf{(DK) Equation}, text=violet
    [{Chapter \ref{ch:weak}}:\\ Weak-error estimate
      [{Sec \ref{ch:GWP}}: Variational solutions]
      [{Sec \ref{sec:ito-Kol}}: It\^o formula]
      [{Sec \ref{sec:weak_error}}: Weak error estimate]
    ]
    [{Chapter \ref{ch:LDP}}:\\ Restricted LDP (Coulomb kernels)
      [{Sec \ref{sec:existence}}: Renormalized\\ kinetic solutions]
      [{Sec \ref{sec-4}}: Exponential estimates]
      [{Sec \ref{sec:LB-LDP}}: Lower bound derivation]
      [{Sec \ref{sec-ws!}}: Skeleton eq.\ analysis]
      [{Sec \ref{sec:LDP-UB}}: Upper bound derivation]
    ]
  ]
\end{forest}
...
\begin{forest}
  ...
  forked edges
  [\textbf{Anderson Nonlinear}\\\textbf{Schr\"odinger (ANLS)}, text=violet
    [{Chapter \ref{ch:ANLS}}:\\ Well-posedness
      [{Sec \ref{sec:AH}}: Anderson Hamiltonian]
      [{Sec \ref{sec:strichartz}}: Strichartz estimates]
      [{Sec \ref{chapter-LWP}}: Local well-posedness]
      [{Sec \ref{chapter:GWP}}: Global well-posedness]
    ]
    [{Chapter \ref{ch:BEC}}:\\ Effective Description
      [{Sec \ref{sec:ANLS-derivation}}: Formal derivation]
      [{Sec \ref{chapter:MFL-MB}}: BBGKY hierarchy]
      [{Sec \ref{sec:depletion}}: Depletion of condensates]
    ]
  ]
\end{forest}
